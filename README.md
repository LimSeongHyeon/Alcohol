...
```
import framework : 0.46s
plugin discovery : 3.14s   (166 plugins)
```

분석을 시작하기도 전에 3.6초. 여기에 실제 이미지의 automagic 스캔과 심볼 테이블 로딩이 더 붙습니다. `vol.py`를 플러그인마다 새로 띄우는 GUI는 사용자가 클릭할 때마다 이 비용을 처음부터 다시 냅니다.
